...
int test_ring_metropolis(const char* path_outfile="histos_Metropolis.root")
{
    ...
    const long long int annealing_flips = 10000*n_spins; 
    /// cluster updates between measurements
    const long long int flips_between_measurements = 10*n_spins; 
    /// number of times to repeat a measurement at each step
    const int n_measurements_per_step = 30e3; 

    IsingLattice lattice(lattice_edge_size, 1000.);
